    ...
    public static double getHighestOffset(double max) {
        for (double i = 0.0D; i < max; i += 0.01D) {
            for (int offset : new int[]{-2, -1, 0, 1, 2}) {
                if (Minecraft.getMinecraft().world.getCollisionBoxes(Minecraft.getMinecraft().player, Minecraft.getMinecraft().player.getEntityBoundingBox().offset(Minecraft.getMinecraft().player.motionX * offset, i, Minecraft.getMinecraft().player.motionZ * offset)).size() > 0) {
                    return i - 0.01D;
                }
            }
        }
